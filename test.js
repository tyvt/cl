...
const start = async () => {
  ...
  DB.runSQL(
    `SELECT * FROM t_topic tt where  create_time != update_time AND fid = "8" LIMIT 1`
  ).then(async (res) => {
    ...
    for await (const iterator of list) {
      ...
      const html = `<h3>${iterator[0]}</h3><br>${data
        ...
        .replace(/&nbsp;/g, "")
        .replace(/iyl-data='http:\/\/a.d\/adblo_ck.jpg'/g, "")
        .replace(/data-link='.*?'/g, "")
      ...
      // await DB.runSQL(
      //   `update t_topic set update_time = "${Math.round(new Date().getTime()/1000)}" where url = "${
      //     iterator[2]
      //   }"`
      // );
      sleep(2000);
